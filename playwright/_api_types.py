...
class RecordHarOptions(ApiType):
    ...
    def __init__(self, path: str, omit_content: bool = None):
        self.path = path
...
class RecordVideoOptions(ApiType):
    ...
    def __init__(self, dir: str, size: IntSize = None):
        self.dir = dir
        self.size = size

    def _to_json(self) -> Dict:
        return filter_out_none(
            {"dir": str(self.dir), "size": self.size._to_json() if self.size else None}
        )
