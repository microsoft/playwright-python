...
class Request(ChannelOwner):
    def __init__(
        ...
        self._fallback_overrides: FallbackOverrides = FallbackOverrides()
    ...
    @property
    def url(self) -> str:
        return self._fallback_overrides.get("url") or self._initializer["url"]
    ...
    @property
    def method(self) -> str:
        return self._fallback_overrides.get("method") or self._initializer["method"]
    ...
    def post_data(self) -> Optional[str]:
        data = self._fallback_overrides.get("post_data") or self.post_data_buffer
        if not data:
            return None
    ...
    def post_data_buffer(self) -> Optional[bytes]:
        ...
        if override:
            return override.encode() if isinstance(override, str) else override
    ...
    def headers(self) -> Headers:
        ...
        if override:
            return RawHeaders(serialize_headers(override)).headers()
    ...
    def _apply_fallback_overrides(self, overrides: FallbackOverrides) -> None:
        self._fallback_overrides = cast(
            FallbackOverrides, {**self._fallback_overrides, **overrides}
        )
...
class Route(ChannelOwner):
    ...
    def fallback(
        ...
        overrides = cast(FallbackOverrides, locals_to_params(locals()))
    ...
    async def continue_(
        ...
        # TODO: debug why we are reported being
        # self._check_not_handled()
        ...
        if postData:
            overrides["post_data"] = postData

        self.request._apply_fallback_overrides(overrides)

        postData = None
        if self.request._fallback_overrides.get("post_data"):
            from_overrides = self.request._fallback_overrides["post_data"]
            if from_overrides is not None:
                postData = (
                    base64.b64encode(from_overrides.encode()).decode()
                    if isinstance(from_overrides, str)
                    else base64.b64encode(from_overrides).decode()
                )
        params: ContinueParameters = {
            "url": self.request._fallback_overrides.get("url"),
            "method": self.request._fallback_overrides.get("method"),
            "headers": serialize_headers(
                cast(Dict[str, str], self.request._fallback_overrides["headers"])
            )
        ...
        await self._race_with_page_close(
            self._channel.send(
                "continue",
                cast(Any, {k: v for k, v in params.items() if v is not None}),
            )
        )
        self._report_handled(True)
