...
def test_should_work(page: Page, server: Server) -> None:
    ...
    page.add_locator_handler(page.locator('text=This interstitial covers the button'), handler)
    ...
    for args in [
        ...
        before_count = 0
        after_count = 0
        page.evaluate('(args) => { window.clicked = 0; window.setupAnnoyingInterstitial(...args); }', args)
...
def test_should_work_with_locator_hover(page: Page, server: Server) -> None:
    ...
    page.add_locator_handler(page.get_by_text('This interstitial covers the button'), lambda: page.locator('#close').click())

    page.locator('#aside').hover()
    page.evaluate('() => { window.setupAnnoyingInterstitial("pointerover", 1, "capture"); }')
    page.locator('#target').hover()
    expect(page.locator('#interstitial')).not_to_be_visible()
    assert page.eval_on_selector('#target', 'e => window.getComputedStyle(e).backgroundColor') == 'rgb(255, 255, 0)'
...
def test_should_throw_when_page_closes(page: Page, server: Server) -> None:
    ...
    page.add_locator_handler(page.get_by_text('This interstitial covers the button'), lambda: page.close())

    page.locator('#aside').hover()
    page.evaluate('() => { window.clicked = 0; window.setupAnnoyingInterstitial("mouseover", 1); }')
...
def test_should_throw_when_handler_times_out(page: Page, server: Server) -> None:
    ...
    page.add_locator_handler(page.get_by_text('This interstitial covers the button'), handler)
...
def test_should_work_with_to_be_visible(page: Page, server: Server) -> None:
    ...
    page.evaluate('() => { window.clicked = 0; window.setupAnnoyingInterstitial("remove", 1); }')
