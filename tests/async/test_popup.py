...
from playwright import HttpCredentials, IntSize
...
async def test_should_inherit_touch_support_from_browser_context(
    ...
    context = await browser.new_context(viewport=IntSize(400, 500), has_touch=True)
...
async def test_should_inherit_viewport_size_from_browser_context(
    ...
    context = await browser.new_context(viewport=IntSize(400, 500))
...
async def test_should_use_viewport_size_from_window_features(browser: Browser, server):
    context = await browser.new_context(viewport=IntSize(700, 700))
    page = await context.new_page()
    await page.goto(server.EMPTY_PAGE)
