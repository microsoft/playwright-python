...
async def test_should_work(page: Page, server: Server) -> None:
    ...
    await page.add_locator_handler(page.locator('text=This interstitial covers the button'), handler)
    ...
    for args in [
        ...
        before_count = 0
        after_count = 0
        await page.evaluate('(args) => { window.clicked = 0; window.setupAnnoyingInterstitial(...args); }', args)
...
async def test_should_work_with_locator_hover(page: Page, server: Server) -> None:
    ...
    await page.add_locator_handler(page.get_by_text('This interstitial covers the button'), lambda: page.locator('#close').click())

    await page.locator('#aside').hover()
    await page.evaluate('() => { window.setupAnnoyingInterstitial("pointerover", 1, "capture"); }')
    await page.locator('#target').hover()
    await expect(page.locator('#interstitial')).not_to_be_visible()
    assert await page.eval_on_selector('#target', 'e => window.getComputedStyle(e).backgroundColor') == 'rgb(255, 255, 0)'
...
async def test_should_throw_when_page_closes(page: Page, server: Server) -> None:
    ...
    await page.add_locator_handler(page.get_by_text('This interstitial covers the button'), lambda: page.close())

    await page.locator('#aside').hover()
    await page.evaluate('() => { window.clicked = 0; window.setupAnnoyingInterstitial("mouseover", 1); }')
...
async def test_should_work_with_to_be_visible(page: Page, server: Server) -> None:
    ...
    await page.evaluate('() => { window.clicked = 0; window.setupAnnoyingInterstitial("remove", 1); }')
